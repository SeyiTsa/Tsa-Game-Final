...
func _ready() -> void:
	...
	for child in $Control/ColorRect.get_children():
		...
		if child is Sprite2D:
			meal_spots.append(child)
func _physics_process(delta: float) -> void:
	$Control/Panel/Label.text = Global.time
	if used_positions:
				...
				for position in used_positions:
					if positions[index] > position:
						...
						if note_markers[position_index].get_child_count() == 0:
							if !note_marker.get_child(0).being_deleted:
...
func meal_made(meal : Meal, counter_spot, total_counter_spots : Array):
	if meal:
		if counter_spot == null:
			displaced_meals.append(meal)
			
		for spot in total_counter_spots:
			if spot == counter_spot:
				var position = total_counter_spots.find(counter_spot)
				meal_spots[position].meal = meal
				meal_spots[position].data = meal.data
				break
		for note in used_note_markers:
			note.get_child(0).being_deleted = true
			used_note_markers.remove_at(0)
			
			if note.get_child_count() > 0:
				var tween = get_tree().create_tween()
				...
				print(meal.data.name)
			break
	else:
		for spot in total_counter_spots:
			if spot == counter_spot:
				var position = total_counter_spots.find(counter_spot)
				meal_spots[position].meal = displaced_meals[0]
				meal_spots[position].data = displaced_meals[0].data
				displaced_meals.remove_at(0)
