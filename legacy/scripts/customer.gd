...
var interactions : Array = ["Seat", "Ordering", "Take Order", "Serve", "Eat", "Favor", "Leave"]
var SPEED : int = 50
	...
	for area in $InteractAreaComponent.get_overlapping_areas():
		if area.is_in_group("meal") and interactable_component.current_interaction == "Serve":
			if area.get_parent().being_thrown:
				if area.get_parent().data.name == current_meal:
					...
					$"Ordered Food".hide()
					get_tree().create_timer(randf_range(15, 20)).timeout.connect(finished_eating)
					area.get_parent().interactable_component.can_be_selected = false
				else:
					await get_tree().create_timer(0.95).timeout
					if $InteractAreaComponent.get_overlapping_areas().has(area):
						area.get_parent().interactable_component.can_be_selected = false
		...
		if !nav.is_navigation_finished():
			...
			match interactable_component.current_interaction:
				"Seat":
					...
					ordering = true
					interactable_component.switch_interaction.emit()
					order_timer.start(order_timer.wait_time + randf_range(-1 , 1))
				"Leave":
					queue_free()
...
func finished_eating():
	...
	interactable_component.switch_interaction.emit()
	seat.occupied = false
	Consts.root.open_seats.append(seat)
	seat = null
	is_sitting = false
	is_talking = false
	has_order_ready = false
